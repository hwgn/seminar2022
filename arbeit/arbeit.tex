...
beschreibt in der Webentwicklung das regelmäßige Abrufen (in einem selbst festgelegten Intervall $\Delta t$) von neuen Inhalten.
...
andernfalls wird der Antwortkörper leer sein (siehe Figuren \ref{fig:polling}, \ref{fig:polling_send}). \cite[Abs. 2.1]{saint-andre_known_2011}
...
Polling ist in der Spezifikation von HTTP nicht definiert, es handelt sich hierbei um eine Implementierung des Clients und des Servers\footnote{Der Autor war zudem nicht in der Lage, eine angemessene wissenschaftliche Arbeit bzw. Definition zu Polling zu finden.}.
...
So könnte dieser zum Beispiel den gesamten abgefragten Inhalt bei jeder Anfrage übermitteln, oder nur Änderungen, die dieser Client noch nicht übermittelt bekommen hat.
...
Dabei wird die Antwort auf eine HTTP-Anfrage zurückgehalten, bis der Server eine Nachricht versenden will (siehe \autoref{fig:long_polling}) \cite[Abs. 2.1]{saint-andre_known_2011} \cite{noauthor_long_2021}.
...
Eine weitere Optimierung des Polling-Konzepts ist das sogenannte \emph{Streaming}, manchmal auch \emph{Comet} oder \emph{Server Push} genannt.
...
der Server terminiert nie die Antwort auf die erste Anfrage (siehe \autoref{fig:streaming}) \cite[Abs. 3]{saint-andre_known_2011}.
...
indem es eine TCP-nahe Verbindung zwischen Client und Server aufbaut und unterstützt (siehe \autoref{fig:websockets}).
...
Für die Datenübermittlung sieht das WebSocket Protokoll ein -- auch in anderen Protokollen ähnlich vertretenes -- Frame-System vor,
also dem Versenden von Daten innerhalb von Frames.
Frames enthalten dabei jeweils einen Header und eine Payload.
Header wiederum enthalten bei WebSockets u. A. den Opcode\footnote{Opcodes (\emph{Operation Codes}, dt. \emph{Befehlscode}) können bei WebSockets Frames mit Text- oder Binärdaten ankündigen, 
...
Der Client benutzt das Vue.js Frontend-Framework mit Vuetify\footnote{UI-Framework, das fertige Komponenten als Grundlage für Webseiten liefert.}, um die Darstellung zu vereinfachen.
...
Im Falle des klassischen Polling gemäß \autoref{sec:polling} wird der Client in regelmäßigen Zeitabständen (hier $\Delta t = 2s$) eine Anfrage an den Server senden, um neue Nachrichten abzufragen.
...
Im Falle einer neuen Nachricht wird die Listener-Funktion \texttt{onmessage} aufgerufen.
...
Dabei wird lediglich der HTTP-Overhead betrachtet, da der TCP/IP-Overhead nicht von der Implementierung beeinflusst werden kann (und in allen Fällen ähnlich sein sollte).
...
Die Request-Payload ist bei Polling-Varianten ein Timestamp, etwa 24 Bytes benötigt\footnote{Es handelt sich um einen Unix-Timestamp gem. ISO 8601 bzw. RFC 3339, welcher in der Regel 24 Zeichen lang ist.}.
...
Die Response hat eine gemessene Größe von 114 Bytes – hier enthält der Response-Header nur den Status-Code und das Datum, da keine Payload übertragen wird.
...
Dieser entsteht bei der \texttt{ws}-Bibliothek durch ein JSON-Objekt, welches den Typ des WebSocket-Packets angibt und einen Overhead von 26 Bytes im Falle einer Nachricht (\texttt{message}) hat.
...
\begin{itemize}
  \item \textbf{Vorteile}
        \begin{itemize}
          \item \textbf{Einfach zu implementieren}: Es sind keine zusätzlichen Bibliotheken notwendig.
          ...
          \item \textbf{Vollständige Akzeptanz bei Browsern}: Polling basiert auf HTTP, welches von allen Browsern unterstützt wird.
          \item \textbf{Geringere Netzwerkbelastung im Falle von hohen Mengen an Nachrichten}: Da bei Polling nur eine Anfrage pro Intervall $\Delta t$ versendet wird,
  ...
  \item \textbf{Nachteile}
        \begin{itemize}
          \item \textbf{Hohe Netzwerkbelastung}: Da bei Polling regelmäßig Anfragen versendet werden, entsteht eine durchgehende Netzwerkbelastung, welche u. U. keine tatsächlichen Informationen übermittelt (Redundanz).
          ...
          \item \textbf{Hoher Header-Overhead für Nachrichtenempfang}: Da bei Polling regelmäßig Anfragen versendet werden, entsteht ein hoher Header-Overhead, welcher für den gesamten Zeitraum der Kommunikation in den regelmäßigen Abfragen besteht.
          \item \textbf{Hohe Server- und Client-Last}: Da bei Polling regelmäßig Anfragen versendet und beantwortet werden müssen, entsteht eine hohe Last auf Server- und Clientseite.
          \item \textbf{Keine Echtzeitübertragung}: Es kann bis zu $\Delta t$ dauern, bis eine Nachricht empfangen wird. Dies kann z. B. bei einer Chatanwendung zu einer unangenehmen Verzögerung führen.
        \end{itemize}
\end{itemize}
...
\begin{itemize}
  \item \textbf{Vorteile}
        \begin{itemize}
          \item \textbf{Reduzierter Header-Overhead}: Da bei Long Polling nur eine Anfrage pro eingehende Nachricht versendet wird, entsteht ein geringerer Header-Overhead als bei Polling.
          \item \textbf{Geringere Server- und Client-Last}: Da bei Long Polling nur eine Anfrage versendet wird, entsteht eine geringere Last auf Server- und Clientseite als bei Polling.
          \item \textbf{Vollständige Akzeptanz bei Browsern}: Long Polling basiert auf HTTP, welches von allen Browsern unterstützt wird.
        \end{itemize}
  \item \textbf{Nachteile}
        \begin{itemize}
          \item \textbf{Timeouts}: Da bei Long Polling eine Anfrage beliebig lang offen bleiben kann, kann es zu Timeouts kommen, also einem Abbruch der Verbindung.
          \item \textbf{Unnötig reservierte Ressourcen}: Betriebssysteme reservieren oft in Antizipation auf eingehende Nachrichten Systemressourcen.
                Diese Ressourcen werden bei Long Polling unnötig lange reserviert. \cite[Abs. 2.2]{saint-andre_known_2011}
          \item \textbf{Verlangsamung bei mehreren Nachrichten in kurzer Zeit}: Werden zwei Nachrichten in kurzer Zeit nacheinander versendet, kann die zweite Nachricht erst empfangen werden, nachdem die erste beim Client eingegangen ist und dieser eine neue Anfrage versendet hat.
...
\begin{itemize}
  \item \textbf{Vorteile}
        \begin{itemize}
          \item \textbf{Weiter reduzierter Header-Overhead}: Streaming benötigt nur eine Anfrage für alle eingehenden Nachrichten, wodurch der Header-Overhead noch weiter reduziert wird.
          \item \textbf{Geringere Server- und Client-Last}: Ähnlich wie bei Long Polling entsteht eine geringere Last auf Server- und Clientseite als bei Polling.
          \item \textbf{Vollständige Akzeptanz bei Browsern}: Streaming basiert auf HTTP, welches von allen Browsern unterstützt wird.
          \item \textbf{Keine Verzögerung bei mehreren Nachrichten in kurzer Zeit}: Da bei Streaming die Nachrichten sofort übertragen werden können, entsteht keine Verzögerung bei mehreren Nachrichten in kurzer Zeit.
        \end{itemize}
  \item \textbf{Nachteile}
        \begin{itemize}
          \item \textbf{Timeouts}: Da bei Streaming eine Anfrage permanent offen gehalten wird, kann es zu Timeouts kommen.
          \item \textbf{Unnötig reservierte Ressourcen}: Bei Streaming werden für die permanent offene Anfrage Systemressourcen reserviert, selbst wenn sie nicht akut benötigt werden. \cite[Abs. 2.2]{saint-andre_known_2011}
          \item \textbf{Möglicherweise nicht unterstützt}: Streaming funktioniert nicht auf allen Systemen --
...
\begin{itemize}
  \item \textbf{Vorteile}
        \begin{itemize}
          \item \textbf{Kein Header-Overhead}: WebSocket-Nachrichten verwenden keine HTTP-Header.
          ...
          \item \textbf{Wesentlich geringere Server- und Client-Last}: Da bei WebSockets Kommunikation beinahe auf TCP-Ebene stattfindet \cite[Abs. 1.5]{melnikov_websocket_2011}, entsteht eine geringere Last als bei Polling-Varianten (vgl. \autoref{tab:data-traffic-comparison}).
          \item \textbf{Echtzeitübertragung}: Nachrichten können in beide Richtungen sofort übertragen werden.
          \item \textbf{Geringste Latenz}: WebSockets haben im Vergleich zu Polling-Varianten die geringste Latenz \cite[Seite 52]{pimentel_communicating_2012}.
          \item \textbf{Kein unsachgemäßer Gebrauch von HTTP}: Anders als Polling-Varianten\footnote{Polling beinhaltet das Versenden von redundanten Anfragen;
  ...
  \item \textbf{Nachteile}
        \begin{itemize}
          \item \textbf{Keine vollständige Akzeptanz bei Browsern}: WebSockets sind ein relativ neues Feature und werden von älteren Browsern nicht unterstützt.
...
Ist die limitierte Verfügbarkeit kein Ausschlusskriterium, fällt die Wahl eindeutig auf WebSockets, 
da diese die geringste Latenz aufweisen und die Nachrichten mit minimalem Overhead übertragen werden können.
...
Muss die Chatanwendung jedoch auf allen Browsern laufen können, dürfen WebSockets nicht verwendet werden\footnote{Zumindest nicht allein. Siehe \autoref{sec:frameworks} für mögliche Alternativen, welche das Problem der Verfügbarkeit durch Fallbacks lösen.}.
